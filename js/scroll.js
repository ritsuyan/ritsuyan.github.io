...
function init() {
    ...
    for(var i = 0 ; i < $inde_arr.length; i++){
        ...
        hammertime.on('tap', function (event) {
            ...
            var $curr_index = $curr.index('.inde');  // get the curr index in li arr ,the first is 0
            $curr.parent().find($('.active_box')).removeClass('active_box');
            $curr.addClass('active_box')

            $('.row_title').html($curr.attr('data-inde'))
            $('.sales_inde').find('a').html($curr.attr('data-inde'))
...
for(var i = 0 ; i < $inde_arr.length; i++){
    var hammertime = new Hammer($inde_arr[i]);
    var curr_inde  =  $inde_arr[i].getAttribute('data-inde');
    hammertime.on('doubletap', function (event) {
        if (toggle.detail === false) {

            var opt_arr = trans_arr(into_detail_page()) // the opt month data
            console.log(opt_arr)
            $('.indContainer').empty();
            $('#main').css({
                'width': '100%',
            ...
            var option = {
                ...
                tooltip: {},
                legend: {
                        data: [curr_inde] //into_detail_page().legend
                    },
                xAxis: {
...
var mock_data = {
    "种类一": {
        "一月": {
            "销售额(元)": "111,222,412",
            "销售额vs目标销售额(元)": "624,123,845",
            "销售额vs去年销售额(元)": "777,222,412",
            "销售额vs国内同类销售额(元)": "624,123,845",
            "销售额vs国际同类销售额(元)": "777,222,412",
            "总处方数(元)": "624,123,845",
            "page2_1(元)": "624,123,845",
            "page2_2(元)": "624,123,845"
        },
        "二月": {
            "销售额(元)": "222,222,412",
            "销售额vs目标销售额(元)": "624,123,845",
...
function into_detail_page() {
    ...
    var curr_inde = $(curr_in).attr('data-inde')  // such as 总处方数
    console.log(curr_inde)
    var curr_push_arr = [];
